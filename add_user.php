...
if ($_SESSION["loggedInUserRole"] == "admin" || $_SESSION["loggedInUserRole"] == "moderator") {
    ...
    if (isset($_POST['saveuser'])) {
        ...
        $role = $_POST['role'];
        $data = db::getRecords("SELECT * FROM user WHERE username='" . $username . "'");
        if (empty($data)) {
            db::insertRecord("INSERT INTO user (`id`,`username`,`password`,`role`) VALUES (NULL,'" . $username . "','" . sha1($password) . "','" . $role . "');");
        }
