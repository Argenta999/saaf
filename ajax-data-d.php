...
if ($_SESSION["loggedInUserRole"] == "admin") {
    $data = db::getRecords("SELECT * FROM data WHERE status = '2' ORDER BY id DESC");
} else {
    $data = db::getRecords("SELECT * FROM data WHERE status = '2' AND user_id='" . $_SESSION["loggedInUserId"] . "' ORDER BY id DESC");
}
